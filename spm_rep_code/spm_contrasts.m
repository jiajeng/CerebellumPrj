...
for i = 1:length(Ic)
        ...
        switch xCon(ic).STAT
            ...
            case {'T','P'}
                ...
                if strcmp(xCon(ic).STAT,'P') && strcmp(SPM.PPM.xCon(ic).PSTAT,'F')
                    ...
                    %------------------------------------------------------
                    xCon(ic).Vcon = struct(...
                        'fname',  ['con_' xCon(ic).name file_ext],... %[sprintf('con_%04d',ic) file_ext]
                        'dim',    SPM.xVol.DIM',...
    ...
    if isempty(xCon(ic).Vspm) || xCon(ic).STAT == 'P'
        ...
        %------------------------------------------------------------------
        xCon(ic).Vspm = struct(...
            'fname',  [sprintf('spm%s_%s',xCon(ic).STAT,xCon(ic).name) file_ext],...%[sprintf('spm%s_%04d',xCon(ic).STAT,ic) file_ext]
            'dim',    SPM.xVol.DIM',...
            'dt',     [spm_type('float32'), spm_platform('bigend')],...
            'mat',    SPM.xVol.M,...
            'pinfo',  [1,0,0]',...
